/*
jsScheme.js - Scheme interpreter in JavaScript, standalone version
...
/*
 * jsScheme 0.1-tae - jsScheme.js
 */

var jsSchemeDebug = false;
var settings;
		 
//FIXME: I broke this, but anyway it wasn't being used. -TE
R = function(s, outputElementId) {
  s='var e=TopEnv;'+s;
  s=eval(s);
  printLog( '*> ' + Str(s) );
  showRes(s, outputElementId);
}
...
// Char object constructor - since we don't have Char type in JS
...
// Symbol object constructor - to distinguish it from String
...
getSymbol = function(name,leaveCase) {
...
Continuation = function(obj,env,cc,f) {
...
pairHtml = function() {
  ...
  for( i=0; i<cells.length; i++ ) {
    if( allSimple || i<1 || (i<2 && firstSymbol) ) {
      s1 += '<td style="background-color:white"'+(cells[i]=='.'?'':edit)
         + (i==0&&firstSymbol ? ' valign=top'+rowSpan : '')
         + '>' + cells[i] + '<\/td>';
    } else {
      s2 += '<tr><td style="background-color:white"'+(cells[i]=='.'?'':edit)
         + '>' + cells[i] + '<\/td><\/tr>';
    }
  }

  return '<table border=0 cellspacing=1 cellpadding=4 style="background-color:#a0a0a0">'
       + '<tr><td valign=top style="background-color:white"'+rowSpan+'>(<\/td>'
       + s1 + '<td valign=bottom style="background-color:white"'+rowSpan+'>)<\/td><\/tr>' + s2 + '<\/table>';
//  onClick=hv(this)
}
...
Lambda = function(args,body,env,compiled) {
  this.args = args;
  this.body = body;
  this.env = env;
...
State = function(obj,env,cc) {
  this.obj = obj;
  this.env = env;
  this.cc = cc;
}

stateEval = function(noTrace) {
...
evalTrue = function(state) {
  state.ready = true;
}

evalVar = function(state) {
...
callF = function( f, args, state ) {
 ...
 Again: while(true) {
  ...
  if( typeof( f ) == 'function' ) {
    ...
    if( f.compiled != false && settings.jit ) {
      try {
        state.ready = true;
...
Syntax = function( transformer, args ) {
...
isEq = function(a,b) { return a==b || isNil(a)&&isNil(b); }
...
TopEnv['ajax-get'] = function(list) {
  var text = jslib.syncGet(list.car);
  doEval (new Pair(list.cdr.car, new Pair(new Pair(theQuote, new
						   Pair(text,theNil)), theNil)), true);
}
...
/*
  dojo.xhrGet({
	...
	error:function(xml, ioArgs){
	alert('Network error!  Please reload this web page.');
      }
    });
}
*/
...
isPair = function(x) { return (x instanceof Pair) && !isNil(x); }
...
clickEval = function(txt, symbolTable, outputElementId) {
  ...
  var time1 = new Date();
  showRes(res, outputElementId);
  if (symbolTable) {
    showSymbols(symbolTable, outputElementId);
  }
  return { result:res, time:((time1-time0)/1000) };
}
var callEval = clickEval;

showRes = function(res, outputElementId) {
  if( res != null ) {
    if( jslib.byId(outputElementId) )
      jslib.byId(outputElementId).innerHTML = Html( res );
  }
}

printLog = function(s,no) {
  if (!settings.log) {
    return;
  }
  settings.log.value += s + (no?'':"\n");
  if( settings.log.doScroll )
    settings.log.doScroll();
}

// Need to wrap alert as calling it from Scheme
// in Firefox otherwise doesn't work
jsAlert = function(text) {
  alert(text)
}

// Need to wrap settimeout as calling it from Scheme
// in Firefox otherwise doesn't work
jsSetTimeout = function(f,t) {
  setTimeout(f,t)
}

init = function(tab, outputElementId, logElementId) {
  settings = { jit: false,       // will we do just-in-time compilation or not?
	       echoInp: true,    // will we log input or not?
	       echoRes: true,    // will we log results or not?
	       log: undefined,   // the log-output element
	       print: true };    // will we print the result and time or not?
  if (jslib) {
    settings.log = jslib.byId(logElementId);
  }
  ShowEnv = TopEnv = new Env(TopEnv);
  if (jsSchemeDebug) {
    showSymbols(tab, outputElementId);
  }
...
// Library end
  ...
  if (typeof console != "undefined") {
    var banner = ["jsScheme 0.1-tae\n",
		  "Copyright (c) 2009 Tom Elam\n",
		  "Copyright (c) 2006 Chris Double\n",
		  "Copyright (c) 2003 Alex Yakovlev\n",
		  "jsScheme may be distributed under the terms of the GNU General Public Licence,",
		  "Version 2; certain other uses are permitted as well.  For details, see the file\n",
		  "`COPYING.txt', which is included in the jsScheme distribution.  There is no\n",
		  "warranty, to the extent permitted by law."];
    jslib.forEach(banner, function(elt) {
	console.debug(elt);
      });
  }
}
...
key1 = function(srcElement) {
  buf1 = srcElement.value;
}

key2 = function(srcElement, symbolTable, txt, timeElement, outputElementId) {
  var buf2 = srcElement.value;
  var re = /(\n|\r\n){2}$/;
  if( !re.test(buf1) && re.test(buf2) ) {
    if( timeElement)
      timeElement.innerHTML = "Elapsed: " + clickEval(txt, symbolTable, outputElementId).time + "s";
    buf1 = buf2;
  }
}

checkEdit = function(srcElement) {
...
showSymbol = function(s, outputElementId) {
  ...
  jslib.byId(outputElementId).innerHTML = s;
}

showSymbols = function(tab, outputElementId) {
  if (!tab || !outputElementId) {
    return;
  }
  ...
  for( i in ShowEnv ) {
    if( i != 'parentEnv' && Env.prototype[i] == undefined ) {
      ...
      var s = '<a href="javascript:showSymbol(\''+i+'\',\''+outputElementId+'\')">'+i+'<\/a>';
...
evalScheme = function(txt) {
    TopParser = new Parser( txt );
    //clickEval(txt); //FIXME: I broke this, but anyway it was only a comment. -TE
...
loadSchemeFile = function(schemeFile) {
  jslib.byId("txt").value = jslib.syncGet(schemeFile, function() {});
}

evalSchemeFile = function(schemeFile) {
  var txtNode = jslib.byId("txt");
  dojo.xhrGet({url:schemeFile,
	       sync:true,
	       load:function(response, ioArgs) { evalScheme(response); },
	       error:function(response, ioArgs) { alert(response); }});
}
